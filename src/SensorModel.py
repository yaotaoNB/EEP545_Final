...
Z_MAX =  0.3 # Weight for max reading
...
class SensorModel:
  ...
  def lidar_cb(self, msg):
    ...
    og_ranges = np.copy(msg.ranges).astype(np.float32)
    og_ranges[og_ranges[:] == 0.0] = self.MAX_RANGE_METERS
    og_ranges[np.isnan(og_ranges)] = self.MAX_RANGE_METERS

    if self.EXCLUDE_MAX_RANGE_RAYS:
      filtered_ranges = og_ranges[og_ranges[:] != self.MAX_RANGE_METERS]
      filtered_angles = self.laser_angles[og_ranges[:] != self.MAX_RANGE_METERS]

      step_size = self.LASER_RAY_STEP*(float(len(filtered_ranges))/float(len(og_ranges))) #adptative step size
      sel_indices = np.arange(0, len(filtered_ranges), step_size).astype(int)
      downsampled_ranges = filtered_ranges[sel_indices]
      self.downsampled_angles = filtered_angles[sel_indices]
    else:
      sel_indices = np.arange(0, len(og_ranges), self.LASER_RAY_STEP).astype(int)
      downsampled_ranges = og_ranges[sel_indices]
      self.downsampled_angles = self.laser_angles[sel_indices]
    ...
    for d in xrange(table_width):
      ...
      for r in xrange(table_width):
        ...
        if r < max_range_px and r >= 0:
          ...
          p_hit = np.exp(-0.5 * ((d - r)**2.0) / (SIGMA_HIT**2.0)) / (np.sqrt(2.0 * np.pi * SIGMA_HIT**2.0))
          
        sensor_model_table[r, d] = Z_MAX * p_max + Z_SHORT * p_short + Z_RAND * p_rand + Z_HIT * p_hit
        norm += sensor_model_table[r, d]
