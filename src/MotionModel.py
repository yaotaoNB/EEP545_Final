...
class KinematicMotionModel:
  ...
  def motion_cb(self, msg):
    ...
    control_noise_speed = np.random.normal(curr_speed, KM_V_NOISE, x.shape)
    control_noise_angle = np.random.normal(curr_steering_angle, KM_DELTA_NOISE, w.shape)
    w_new = w + control_noise_speed / self.CAR_LENGTH * np.tan(control_noise_angle) * (msg.header.stamp - self.last_vesc_stamp).to_sec()
    x += (self.CAR_LENGTH / np.tan(control_noise_angle) * (np.sin(w_new) - np.sin(w)))
    y += (self.CAR_LENGTH / np.tan(control_noise_angle) * (-np.cos(w_new) + np.cos(w)))
    
    x = np.random.normal(x, KM_X_FIX_NOISE, x.shape)
    y = np.random.normal(y, KM_Y_FIX_NOISE, y.shape)
    w_new = np.random.normal(w_new, KM_THETA_FIX_NOISE, w_new.shape)
    
    self.particles[:,0] = x
    self.particles[:,1] = y
    self.particles[:,2] = w_new
    
    for i in range(self.particles.shape[0]):
      if self.particles[i, 2]  < -1*np.pi:
...
TEST_STEERING_ANGLE = 0.75 # radians
TEST_DT = 2 # seconds
