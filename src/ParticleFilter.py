...
class ParticleFilter():
  ...
  def expected_pose(self):
    ...
    computed_theta = np.arctan2(np.sum(np.sin(theta) * weights), np.sum(np.cos(theta) * weights))
    return np.array([np.sum(x * weights), np.sum(y * weights), computed_theta])
  ...
  def visualize(self):
    ...
    if self.pub_laser.get_num_connections() > 0 and isinstance(self.sensor_model.last_laser, LaserScan):
      self.sensor_model.last_laser.header.frame_id = "/laser" #"map" 
